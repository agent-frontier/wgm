...
| A repository change is terminal | One baseline, never re-read, covering HEAD/branch, the stash, tracked+untracked+ignored paths, and diff content — because committing, stashing, or writing an ignored file all leave `git status` clean. Any change aborts the audit with no retry, prints the exact delta, and reverts nothing. |
