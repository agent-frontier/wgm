...
- **The one mental model:** wgm steers on **backpressure** — a deterministic pass/fail check. Your
  job is to sit *on* the loop (add signs, set thresholds), not *in* it.
...
| [running-the-loop.md](running-the-loop.md) | `loop.sh`, Ralph-lite vs full, thresholds, model escalation | Driving a large or autonomous build |
