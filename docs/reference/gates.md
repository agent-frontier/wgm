...
| `test-audit.sh` | Docs-audit dispatch: argument errors, dry-run, four independent personas, writer-after-four ordering, a failed/empty/banner persona blocking the writer, report-contract validation for personas and the writer, terminal (never-retried) mutation detection including a role that commits, stashes, or writes a gitignored path, bounded retries, the timeout bound and its cooperative fallback, the non-git refusal and `--allow-unguarded`, concurrency locking, report placement, argv/stdin agent modes |
