...
**Date:** 2026-06-16 · **Status:** documentation shipped; **Tier 1 of the roadmap shipped** in
`feat/roadmap-tier1-governance` (SKILL.md v0.3); Tier 2–3 deferred to follow-up PRs.

> The first pass shipped **documentation only** — operator executive overviews, an
> [operator landing page](../operator/README.md), and this analysis. **Tier 1 below is now
> implemented** (constitution, consistency gate, no-placeholder discipline, loop guardrails);
> Tier 2–3 remain a proposed roadmap, each to land as its own branch and PR.
...
## Proposed roadmap (deferred to follow-up PRs)
...
### Tier 2 — loop.sh + config ergonomics
...
6. **Memories with a token budget** *(Ralph Orchestrator)* — an append-only `.wgm/memories.md` of
   stall patterns and gotchas, read within a ~2000-token budget at each Analyze step; also enforce
   the already-recommended `.wgm/scores.md` trajectory log.
7. **Hard operational limits** *(Ralph Orchestrator)* — add `--max-runtime-seconds`,
