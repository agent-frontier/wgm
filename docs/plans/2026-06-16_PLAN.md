...
**Date:** 2026-06-16 · **Status:** **all Tier 1–3 items shipped** to `main` (SKILL.md v0.3), plus a
token-economy pass (CONTEXT.md glossary + compact agent-only single-token/TOON encoding). The roadmap
below is complete. The [Ralph landscape](2026-06-16_RALPH_LANDSCAPE.md) doc is a **point-in-time
competitive snapshot, not a living tracker** — for what wgm has actually assimilated since, see
[`references/heuristics.md`](../../references/heuristics.md)'s per-entry **Provenance** fields.

> The first pass shipped **documentation only** — operator executive overviews, an
> [operator landing page](../operator/README.md), and this analysis. **Every roadmap item has since
> landed** (constitution, consistency gate, no-placeholder discipline, loop guardrails, operational
> limits, memories, `wgm.yml` gates, scale-adaptive tracks, subagent two-stage review, and the
> CONTEXT.md glossary), each as its own branch and PR.
...
### Tier 2 — loop.sh + config ergonomics — shipped
5. **Declarative `wgm.yml` gates** *(Ralph Orchestrator)* — **shipped:** an optional config with named
...
8. **Scale-adaptive tracks** *(BMAD-METHOD)* — **shipped:** Triage auto-classifies Quick / Standard /
   Full and right-sizes the ceremony (a one-file fix skips scenarios and preflight; a greenfield app
   gets the full rig).

### Tier 3 — structural / larger — shipped
9. **Per-task subagent dispatch with two-stage review** *(Superpowers)* — **shipped:** dispatch
   implementer, spec-compliance reviewer, and quality reviewer subagents per task, each with curated
   context and a typed status protocol (done / done-with-concerns / needs-context / blocked); pick
   the model by task complexity.
10. **CONTEXT.md domain glossary** *(mattpocock)* — **shipped:** a living terminology glossary
    maintained during Grill, kept separate from specs and consulted each iteration to enforce naming
    consistency and cut token cost. Shipped alongside it: a **token-economy** convention letting
    agent-only `.wgm/` artifacts use **single-token keys** (kanji is one instantiation — verified
    per model tokenizer) serialized as TOON with an embedded legend, to min-max context while
    human-facing artifacts stay readable.
